...
void apply_move(ChessBoard &chess, string move, string color) {
	BitBoard crt_player;
	int col;
	if(color == "white") {
	...
	if ((((move == "e1g1") || (move == "e1c1")) && (k1 & chess.white_king.current_form)) 
		...
		if(destination_str[0] == 'c' && color == "white"){
			chess.crt_hash ^= tt.z.pieces[ROOKS][1][3];
			chess.crt_hash ^= tt.z.pieces[ROOKS][1][0];
			chess.crt_hash ^= tt.z.pieces[KINGS][1][2];
			chess.crt_hash ^= tt.z.pieces[KINGS][1][4];
		} else if(destination_str[0] == 'g' && color == "white"){
			chess.crt_hash ^= tt.z.pieces[ROOKS][1][7];
			chess.crt_hash ^= tt.z.pieces[ROOKS][1][5];
			chess.crt_hash ^= tt.z.pieces[KINGS][1][6];
			chess.crt_hash ^= tt.z.pieces[KINGS][1][4];
		} else if(destination_str[0] == 'c' && color == "black"){
			chess.crt_hash ^= tt.z.pieces[ROOKS][0][59];
			chess.crt_hash ^= tt.z.pieces[ROOKS][0][56];
			chess.crt_hash ^= tt.z.pieces[KINGS][0][58];
			chess.crt_hash ^= tt.z.pieces[KINGS][0][60];
		} else if(destination_str[0] == 'g' && color == "black"){
			chess.crt_hash ^= tt.z.pieces[ROOKS][0][63];
			chess.crt_hash ^= tt.z.pieces[ROOKS][0][61];
			chess.crt_hash ^= tt.z.pieces[KINGS][0][62];
			chess.crt_hash ^= tt.z.pieces[KINGS][0][60];
		} 
	} else {
		if (is_pawn == false) {
	...
	if(is_pawn && (destination & chess.black_board.en_passant)){
		chess.crt_hash ^= tt.z.pieces[PAWNS][0][dbit - 8];
	} else if(is_pawn && (destination & chess.white_board.en_passant)) {
		chess.crt_hash ^= tt.z.pieces[PAWNS][1][dbit + 8];
	}
	...
	if (crt_player.castled ^ bcastled) {
		if(col == 1)chess.crt_hash ^= tt.z.white_castle[3];
		else chess.crt_hash ^= tt.z.black_castle[3];
	} else {
		if (crt_player.first_move_king ^ bfirst_move_king) {
			if ((crt_player.first_move_rook1 ^ bfirst_move_rook1) && (crt_player.first_move_rook2 ^ bfirst_move_rook2)) {
				if(col == 1)chess.crt_hash ^= tt.z.white_castle[0];
				else chess.crt_hash ^= tt.z.black_castle[0];
			} else if (crt_player.first_move_rook1 ^ bfirst_move_rook1) {
				if(col == 1)chess.crt_hash ^= tt.z.white_castle[2];
				else chess.crt_hash ^= tt.z.black_castle[2];
			} else if (crt_player.first_move_rook2 ^ bfirst_move_rook2) {
				if(col == 1)chess.crt_hash ^= tt.z.white_castle[1];
				else chess.crt_hash ^= tt.z.black_castle[1];
			}
		}
	}
...
int alfabeta_negamax (ChessBoard &chess, int player, int alfa, int beta, int depth, bool allowNull) {
	...
	if (tt.exists(chess.crt_hash)) {
		HashEntry tte = tt.getEntry(chess.crt_hash);
		if (tte.depth >= depth) {
			if (tte.flag == EXACT) {
				return tte.value;
	...
	if (depth == 0) {
		...
		if (eval <= alfa) {
			entry.init(chess.crt_hash, depth, eval, LOWER);
			tt.add(chess.crt_hash, entry);
		} else if (eval >= beta) {
			entry.init(chess.crt_hash, depth, eval, UPPER);
			tt.add(chess.crt_hash, entry);
		} else {
			entry.init(chess.crt_hash, depth, eval, EXACT);
			tt.add(chess.crt_hash, entry);
		}
		return eval;
	}
	...
	if (score <= alfa) {
		entry.init(chess.crt_hash, depth, score, LOWER);
		tt.add(chess.crt_hash, entry);
	} else if (score >= beta) {
		entry.init(chess.crt_hash, depth, score, UPPER);
		tt.add(chess.crt_hash, entry);
	} else {
		entry.init(chess.crt_hash, depth, score, EXACT);
		tt.add(chess.crt_hash, entry);
	}
